...
//player detection
     ...
     void shooting()
    {
        if(hasGun&& playerNearBy)
        {
            ...
            if(rightSide)
            {
            Instantiate(rifleBullet,currentGun.transform.position+new Vector3(-2.9f,0.15f,0),rifleBullet.transform.rotation);
            }
            else{
            Instantiate(rifleBullet,currentGun.transform.position+new Vector3(2.8f,0.15f,0),rifleBullet.transform.rotation);
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.gameObject.CompareTag("rocketGun"))
        {
